...
public class TtsDemo extends Activity implements OnClickListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		// 情感列表
		mEmotEntries = getResources().getStringArray(R.array.emot_entries);
		mEmotValue = getResources().getStringArray(R.array.emot_values);
	...
	private void initLayout() {
		...
		findViewById(R.id.tts_btn_emot_select).setOnClickListener(TtsDemo.this);
...
//			int code = mTts.synthesizeToUri(text, path, mTtsListener);
	...
	private void showEmotSelectDialog() {
		...
		// 选择在线合成
		case R.id.tts_radioCloud:			
			new AlertDialog.Builder(this).setTitle("在线合成情感选项(目前仅小艾支持)")
				.setSingleChoiceItems(mEmotEntries, // 单选框有几项,各是什么名字
						emotSelectedNum, // 默认的选项
						new DialogInterface.OnClickListener() { // 点击单选框后的处理
					public void onClick(DialogInterface dialog,
							int which) { // 点击了哪一项
						emot = mEmotValue[which];
						emotSelectedNum = which;
						dialog.dismiss();
					}
				}).show();
			break;
